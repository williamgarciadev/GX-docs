---
title: "ContextPath Rewrite: X-Forwarded Headers Support (App behind Reverse Proxy)"
source_id: 45983
source_url: https://wiki.genexus.com/commwiki/wiki?45983
genexus_version: "18"
---

# ContextPath Rewrite: X-Forwarded Headers Support (App behind Reverse Proxy)
...
To provide accurate information to the Web Server, use these standard HTTP headers:
...
Application hosted on Web Server app: app1

* http://webserver/app1

It needs to be accessible via a Reverse Proxy, as follows:

* http://public-host/subfolder/apps/app1

The problem here is that the Working Application does not know it is acting as a server to the outside world by context path: "/subfolder/apps/app1".

So all the Relative host URLs returned by the App are "/app1" and not "/subfolder/apps/app1", which can lead to access problems from the outside world.

This is something known as Rewriting the Context Path.

There is a draft RFC request to standardize a new HTTP Header for this scenario: "X-Forwarded-Prefix". However, it is not yet supported by many Web Servers.

One solution to this problem is to include a filter that reMaps the requestPath automatically.
...
2. Put the JAR file on web server lib.
...
As an alternative approach, you can set the environment variable HTTP\_FORWARDEDHEADERS\_ENABLED="true", which enables the processing of forwarded headers. After setting this variable, configure the reverse proxy to forward the appropriate headers.
...
| --- |
| **Backlinks** |
| [ContextPath Rewrite: X-Forwarded Headers Support (App behind Reverse Proxy) (GeneXus 18 Upgrade 10)](https://wiki.genexus.com/commwiki/wiki?59271) | [RemoteAddr function](https://wiki.genexus.com/commwiki/wiki?8484) |
