---
title: "HowTo: Authenticate to Azure AD using OpenID Connect with GAM (GeneXus 18 Upgrade 8 or prior)"
source_id: 60566
source_url: https://wiki.genexus.com/commwiki/wiki?60566
genexus_version: "18"
---

# HowTo: Authenticate to Azure AD using OpenID Connect with GAM (GeneXus 18 Upgrade 8 or prior)
...
`[imagen omitida: wiki id 55126]`

Take this URL and make a request in the browser or Postman.

`[imagen omitida: wiki id 55127]`

In this screen, Azure AD will give you six certificates and you can use any of them.

**Important**

Download the certificates published by Azure AD and configure them manually in GAM. Certificates are available at the JWKS endpoint exposed by Azure.

When configuring certificates manually, keep in mind that Azure periodically rotates its signing certificates. The token header includes a field called kid, which indicates the certificate that must be used to validate the signature.

Make sure the certificate you configure in GAM matches the one currently active for the kid in the token. If Azure rotates the keys, you need to manually update the certificate in GAM to continue validating tokens correctly.
